...
export function VideoListItem({
  ...
  const handleDownloadType = (type: "video" | "audio") => {
    setDownloadOpen(false)

    if (!user) {
      sessionStorage.setItem("pendingAction", JSON.stringify({ type: "download", contentId: id, contentType: type }))
    ...
    const downloadUrl = type === "video" ? videoDownloadUrl : audioDownloadUrl
          ...
          <div className="flex items-center gap-0.5 mt-1" onClick={handleButtonClick}>
            <DropdownMenu open={downloadOpen} onOpenChange={setDownloadOpen}>
              ...
              <DropdownMenuContent align="end" className="w-48">
                <DropdownMenuItem onClick={() => handleDownloadType("video")} className="cursor-pointer">
                  Download Video
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleDownloadType("audio")} className="cursor-pointer">
                  Download Audio
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
