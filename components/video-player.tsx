...
export function VideoPlayer({
  ...
  useEffect(() => {
    const fetchLatestVideos = async () => {
      try {
        const videosQuery = query(collection(db, "content"), orderBy("uploadedAt", "desc"), limit(10))
        const querySnapshot = await getDocs(videosQuery)
        const videos: RelatedVideo[] = querySnapshot.docs.map((doc) => {
          const data = doc.data()
  ...
  useEffect(() => {
    ...
    if (playerRef.current && playerRef.current.loadVideoById) {
      console.log("[v0] Loading new video in existing player")
    ...
    try {
      playerRef.current = new (window as any).YT.Player("youtube-player", {
        ...
        playerVars: {
          ...
          fs: 0, // Hide fullscreen button from YouTube
          enablejsapi: 1,
        },
        events: {
  ...
  const handleDownload = (type: "video" | "audio") => {
    ...
    if (!user) {
      ...
      sessionStorage.setItem(
        ...
        JSON.stringify({
          ...
          audioDownloadUrl: currentVideo.audioDownloadUrl,
        }),
      )
    ...
    if (!hasActiveSubscription && !isAdmin) {
      console.log("[v0] User needs subscription, showing modal")
      setShowSubscriptionModal(true)
      return
    }

    const downloadUrl = type === "video" ? currentVideo.videoDownloadUrl : currentVideo.audioDownloadUrl

    if (downloadUrl) {
      console.log("[v0] Starting download:", downloadUrl)
      const link = document.createElement("a")
      link.href = downloadUrl
      link.download = `${currentVideo.title}.${type === "video" ? "mp4" : "mp3"}`
      link.target = "_blank"
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
    } else {
      alert(`${type === "video" ? "Video" : "Audio"} download link not available`)
    }
  }
        ...
        <div className="flex flex-col lg:flex-row gap-3 md:gap-6 md:p-4 pb-20 md:pb-24">
          <div className="flex-1 min-w-0">
            <div
              ...
              className="relative bg-black md:rounded-xl overflow-hidden aspect-video mb-2 md:mb-4 group"
              onMouseMove={handleMouseMove}
              onMouseLeave={handleMouseLeave}
            >
              <div id="youtube-player" className="w-full h-full" />

              {showAd && <VideoAdOverlay onSkip={handleSkipAd} skipDelay={5} />}

              <div
                className={`absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-black/40 transition-opacity duration-200 ${showControls ? "opacity-100" : "opacity-0 pointer-events-none"}`}
              >
                <div className="absolute inset-0 flex items-center justify-center">
                  <Button
                    ...
                    variant="ghost"
                    onClick={togglePlay}
                    className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-green-600/90 hover:bg-green-500/90 border-2 border-green-400"
                  >
                    {isPlaying ? (
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 md:p-4 space-y-2">
                  <div className="flex items-center gap-2">
                    <span className="text-white text-xs md:text-sm font-medium min-w-[40px]">
                    ...
                    <Slider
                      ...
                      className="flex-1 [&_[role=slider]]:bg-green-500 [&_[role=slider]]:border-green-400"
                    />
                    <span className="text-white text-xs md:text-sm font-medium min-w-[40px]">
                      {formatTime(duration)}
                    </span>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1 md:gap-2">
                      <Button
                        ...
                        variant="ghost"
                        onClick={togglePlay}
                        className="h-8 w-8 md:h-10 md:w-10 text-white hover:bg-green-600/50"
                      >
                        {isPlaying ? (
                      ...
                      <div className="flex items-center gap-2 group/volume">
                        ...
                        <div className="hidden md:block w-0 group-hover/volume:w-20 transition-all duration-300 overflow-hidden">
                          <Slider
                            value={[volume]}
                            max={100}
                            step={1}
                            onValueChange={handleVolumeChange}
                            className="w-20 [&_[role=slider]]:bg-green-500 [&_[role=slider]]:border-green-400"
                          />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1 md:gap-2">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <div className="px-2 py-1.5 text-xs font-semibold">Quality</div>
                          {["1080p", "720p", "480p", "360p", "240p"].map((q) => (
                            <DropdownMenuItem
                              key={q}
                              onClick={() => changeQuality(q)}
                              className={`cursor-pointer ${quality === q ? "bg-green-100 text-green-700" : ""}`}
                            >
                              {q} {quality === q && "✓"}
                            </DropdownMenuItem>
                          ))}
              ...
              <div className="flex items-center gap-1.5 md:gap-2 flex-wrap mb-3 md:mb-4">
                ...
                <DropdownMenu open={showShareMenu} onOpenChange={setShowShareMenu}>
                  <DropdownMenuTrigger asChild>
                    <Button
                      size="sm"
                      variant="ghost"
                      className="bg-green-100 hover:bg-green-200 text-green-700 rounded-full px-2 md:px-4 h-7 md:h-9 text-xs md:text-sm"
                    >
                ...
                <DropdownMenu open={showDownloadMenu} onOpenChange={setShowDownloadMenu}>
                  ...
                  <DropdownMenuContent align="start" className="w-40">
                    <DropdownMenuItem onClick={() => handleDownload("video")} className="cursor-pointer">
                      Download Video
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => handleDownload("audio")} className="cursor-pointer">
                      Download Audio
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
                ...
                <DropdownMenu open={showMoreMenu} onOpenChange={setShowMoreMenu}>
                  <DropdownMenuTrigger asChild>
                    <Button
                      size="icon"
                      variant="ghost"
                      className="bg-green-100 hover:bg-green-200 text-green-700 rounded-full px-2 md:px-3 h-7 md:h-9"
                    >
                      <MoreHorizontal className="w-3 h-3 md:w-4 md:h-4" />
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end" className="w-48">
                    <DropdownMenuItem onClick={() => setShowDownloadMenu(true)} className="cursor-pointer">
          ...
          <div className="lg:w-[320px] xl:w-[360px] px-3 md:px-0">
            <h2 className="text-sm md:text-base font-semibold mb-2 md:mb-3 text-green-700">LATEST</h2>
